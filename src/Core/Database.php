...
class Database
{
            ...
            if ($this->config['driver'] === 'sqlite') {
                ...
                if (!is_dir($dbDir)) {
                    if (!mkdir($dbDir, 0755, true)) {
                        throw new \Exception("Unable to create database directory: $dbDir");
                    }
                }
    ...
    {
        // Usar schema MySQL se driver for mysql, senão usar SQLite
    ...
    private function createSQLiteTables(): void
    {
        ...
        -- Grupos de opções de produtos (ex: Tamanho, Sabor, Complementos)
        CREATE TABLE IF NOT EXISTS product_option_groups (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        -- Opções de produtos (ex: P, M, G para Tamanho)
        CREATE TABLE IF NOT EXISTS product_options (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        CREATE TABLE IF NOT EXISTS order_items (
            ...
            options TEXT, -- JSON com opções selecionadas
            FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE,
            FOREIGN KEY (product_id) REFERENCES products(id)
        );
        ...
        ";
         ...
         // Criar tabela para notificações push
        $this->pdo->exec("
            CREATE TABLE IF NOT EXISTS push_subscriptions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                establishment_id INTEGER NOT NULL,
